...
import com.cgnpc.bbxpark.common.utils.Base64DecodedMultipartFileUtil;
...
public class FileCenterController extends BaseController {
    ...
    @ApiOperation(value = "文件上传.base64")
    @ResponseBody
    @PostMapping(value = "base64/upload")
    public CudResult<FileModel> base64Upload(@RequestPart("file") String file, @RequestPart("fileName") String fileName) {
        // 测试专为base64在转回来
//        MultipartFile multipartFile = Base64DecodedMultipartFile.base64ToMultipartFile(Base64DecodedMultipartFile.encodeFileToBase64(file));
        Long tenantId = WebFrameworkUtils.getHeaderTenantId();
        FileModel fileModel = fileCenterService.upload(Base64DecodedMultipartFileUtil.base64ToMultipartFile(file), fileName,tenantId);
