...
@Slf4j
public class RestaurantWasteRecordListener extends AnalysisEventListener<RestaurantWasteRecordParam> {
    ...
    @Getter
    private List<RestaurantWasteRecordParam> addList = new ArrayList<>();
     ...
     */
    @Override
    public void invoke(RestaurantWasteRecordParam param, AnalysisContext analysisContext) {
        // 获取当前解析行的索引（0开始）
        ...
        try {
            ...
            if(ObjectUtil.isEmpty(param.getHandleTimeStr()) || ObjectUtil.isEmpty(param.getQuantity()) || ObjectUtil.isEmpty(param.getName())){
                return;
            }
        } catch (Exception e) {
            log.info("餐料垃圾处理信息第{}行解析失败：{}", currentRowIndex + 1 , e.getMessage());
        }
        addList.add(param);
    }
