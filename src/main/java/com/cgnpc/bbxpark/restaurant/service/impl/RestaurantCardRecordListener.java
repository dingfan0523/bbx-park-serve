...
@Slf4j
public class RestaurantCardRecordListener extends AnalysisEventListener<RestaurantCardRecordParam> {
    ...
    @Getter
    private List<RestaurantCardRecordParam> addList = new ArrayList<>();
     ...
     */
    @Override
    public void invoke(RestaurantCardRecordParam param, AnalysisContext analysisContext) {
        // 获取当前解析行的索引（0开始）
        ...
        try {
            ...
            if(ObjectUtil.isEmpty(param.getOccurTimeStr()) || ObjectUtil.isEmpty(param.getAccountBalance()) || ObjectUtil.isEmpty(param.getTradeAmount())){
                return;
            }
        } catch (Exception e) {
            log.info("一卡通消费信息第{}行解析失败：{}", currentRowIndex + 1 , e.getMessage());
        }
        addList.add(param);
    }
