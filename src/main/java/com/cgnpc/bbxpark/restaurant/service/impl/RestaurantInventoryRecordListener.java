...
@Slf4j
public class RestaurantInventoryRecordListener extends AnalysisEventListener<RestaurantInventoryRecordParam> {
    ...
    @Getter
    private List<RestaurantInventoryRecordParam> addList = new ArrayList<>();
     ...
     */
    @Override
    public void invoke(RestaurantInventoryRecordParam param, AnalysisContext analysisContext) {
        // 获取当前解析行的索引（0开始）
        ...
        try {
            ...
            if(ObjectUtil.isEmpty(param.getStockQuantity()) || ObjectUtil.isEmpty(param.getName()) || ObjectUtil.isEmpty(param.getCategory())){
                return;
            }
        } catch (Exception e) {
            log.info("餐料库存信息第{}行解析失败：{}", currentRowIndex + 1 , e.getMessage());
        }
        addList.add(param);
    }
