...
@Slf4j
public class VehicleApplyListener extends AnalysisEventListener<VehicleApplyParam> {
    ...
    @Getter
    private List<VehicleApplyParam> addList = new ArrayList<>();
     ...
     */
    @Override
    public void invoke(VehicleApplyParam param, AnalysisContext analysisContext) {
        // 获取当前解析行的索引（0开始）
        ...
        try {
            ...
            log.info("电召车出车记录第{}行解析失败：{}", currentRowIndex + 1 , e.getMessage());
        }
        addList.add(param);
    }
