...
@Slf4j
public class DwdVehicleMonthlySettlementListener extends AnalysisEventListener<DwdVehicleMonthlySettlementParam> {
    ...
    @Getter
    private List<DwdVehicleMonthlySettlementParam> addList = new ArrayList<>();
     ...
     */
    @Override
    public void invoke(DwdVehicleMonthlySettlementParam param, AnalysisContext analysisContext) {
        // 获取当前解析行的索引（0开始）
        ...
        try {
            ...
            if(ObjectUtil.isEmpty(param.getCarPlate())){
                return;
            }
        } catch (Exception e) {
            log.info("车辆费用结算信息第{}行解析失败：{}", currentRowIndex + 1 , e.getMessage());
        }
        addList.add(param);
    }
