...
@Slf4j
public class DwdVehicleCarRentApplyListener extends AnalysisEventListener<DwdVehicleCarRentApplyParam> {
    ...
    @Getter
    private List<DwdVehicleCarRentApplyParam> addList = new ArrayList<>();
     ...
     */
    @Override
    public void invoke(DwdVehicleCarRentApplyParam param, AnalysisContext analysisContext) {
        // 获取当前解析行的索引（0开始）
        ...
        try {
            ...
            if(ObjectUtil.isEmpty(param.getApplicant())){
                return;
            }
        } catch (Exception e) {
            log.info("租车记录第{}行解析失败：{}", currentRowIndex + 1 , e.getMessage());
        }
        addList.add(param);
    }
