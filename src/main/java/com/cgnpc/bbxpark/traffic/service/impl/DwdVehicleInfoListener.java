...
@Slf4j
public class DwdVehicleInfoListener extends AnalysisEventListener<DwdVehicleInfoParam> {
    ...
    @Getter
    private List<DwdVehicleInfoParam> addList = new ArrayList<>();
     ...
     */
    @Override
    public void invoke(DwdVehicleInfoParam param, AnalysisContext analysisContext) {
        // 获取当前解析行的索引（0开始）
        ...
        try {
            ...
            if(ObjectUtil.isEmpty(param.getLicensePlate())){
                return;
            }
        } catch (Exception e) {
            log.info("车辆信息第{}行解析失败：{}", currentRowIndex + 1 , e.getMessage());
        }
        addList.add(param);
    }
