...
@Slf4j
public class DwdVehiclePartReplaceListener extends AnalysisEventListener<DwdVehiclePartReplaceParam> {
    ...
    @Getter
    private List<DwdVehiclePartReplaceParam> addList = new ArrayList<>();
     ...
     */
    @Override
    public void invoke(DwdVehiclePartReplaceParam param, AnalysisContext analysisContext) {
        // 获取当前解析行的索引（0开始）
        ...
        try {
            ...
            log.info("轮胎更换记录第{}行解析失败：{}", currentRowIndex + 1 , e.getMessage());
        }
        addList.add(param);
    }
