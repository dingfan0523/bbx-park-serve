...
@Slf4j
public class VehicleLineOrderInfoListener extends AnalysisEventListener<VehicleLineOrderInfoParam> {
    ...
    @Getter
    private List<VehicleLineOrderInfoParam> addList = new ArrayList<>();
     ...
     */
    @Override
    public void invoke(VehicleLineOrderInfoParam param, AnalysisContext analysisContext) {
        // 获取当前解析行的索引（0开始）
        ...
        try {
            ...
            log.info("便民班车订单表第{}行解析失败：{}", currentRowIndex + 1 , e.getMessage());
        }
        addList.add(param);
    }
