...
@Slf4j
public class DwdVehicleDriverInfoListener extends AnalysisEventListener<DwdVehicleDriverInfoParam> {
    ...
    @Getter
    private List<DwdVehicleDriverInfoParam> addList = new ArrayList<>();
     ...
     */
    @Override
    public void invoke(DwdVehicleDriverInfoParam param, AnalysisContext analysisContext) {
        // 获取当前解析行的索引（0开始）
        ...
        try {
            ...
            if(ObjectUtil.isEmpty(param.getDriverName())){
                return;
            }
        } catch (Exception e) {
            log.info("司机表第{}行解析失败：{}", currentRowIndex + 1 , e.getMessage());
        }
        addList.add(param);
    }
