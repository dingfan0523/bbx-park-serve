...
@Slf4j
public class DwdVehicleCarTaskRecordListener extends AnalysisEventListener<DwdVehicleCarTaskRecordParam> {
    ...
    @Getter
    private List<DwdVehicleCarTaskRecordParam> addList = new ArrayList<>();
     ...
     */
    @Override
    public void invoke(DwdVehicleCarTaskRecordParam param, AnalysisContext analysisContext) {
        // 获取当前解析行的索引（0开始）
        ...
        try {
            ...
            log.info("车辆行驶记录第{}行解析失败：{}", currentRowIndex + 1 , e.getMessage());
        }
        addList.add(param);
    }
