...
//			throw Exceptions.unchecked(e);
	...
	public static String generateSalt() {
		return StringUtils.replace(UUID.randomUUID().toString(), "-", "").substring(0, 15);
	}

	public static String genPassword(String password, String salt) {
		String property = System.getProperty("matrix.password.use-md5-no-salt");
		if ("true".equalsIgnoreCase(property)) {
			return DigestUtil.md5DigestAsHex(password.getBytes());
		} else {
			return DigestUtil.sha1Hex(password + salt);
		}
	}
