...
@Slf4j
public class ProblemReportImportListener extends AnalysisEventListener<ProblemReportImportParam> {

    // 表头行号（和代码中headRowNumber一致，比如3）
    private static final int HEAD_ROW_NUM = 0;

    @Getter
    private List<ProblemReportImportParam> addList = new ArrayList<>();
     ...
     */
    @Override
    public void invoke(ProblemReportImportParam param, AnalysisContext analysisContext) {
        // 获取当前解析行的索引（0开始）
        ...
        try {
            ...
            if(ObjectUtil.isEmpty(param.getProblemDesc()) || ObjectUtil.isEmpty(param.getCreateBy())){
                return;
            }
        } catch (Exception e) {
            log.info("报事报修信息第{}行解析失败：{}", currentRowIndex + 1 , e.getMessage());
        }
        addList.add(param);
    }
